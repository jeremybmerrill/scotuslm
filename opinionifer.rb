...
def formatOpinion(opinion, lineCombiner = LineCombiner.new)
  ...
  newOpinion = newOpinion.join(" ").gsub("[", "").gsub("]", "").gsub("(","").gsub(")", "").split(/[?.!:] /)
  lines = [""]
  newOpinion.each do |line|
    #if the number of multi-letter words is not greater than two
    if line.size == 1 || line.split(" ").size == 1 || line[0] == "§" || lines[-1].split(" ")[-1]  == "v"
      lines[-1] += ". " + line
    else
    ...
    cases.each do |caseFolder|
      ...
      orderedPages.each_with_index do |filename, index|
        ...
        if isBeginning[0] || index == pages.size-1
          ...
          begin
            File.open(opinionFilepath, "w") {|f| f.write(formatOpinion(opinion, lineCombiner)) }
